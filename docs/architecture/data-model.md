...
3. ~~**¿Una `Transaction` puede quedar sin `Category` (nullable), o siempre se asigna una categoría "Sin categorizar" creada por defecto?**~~ **Resuelta (2026-07-22)**: categoría por defecto — `Transaction.category` es obligatoria y cada usuario tiene una categoría "Sin categorizar" creada por defecto. Ver [ADR-0002](../decisions/0002-prisma-domain-mapping.md). **Sin efecto desde 2026-08-14**: retirado el ledger ([ADR-0007](../decisions/0007-retire-ledger-surface.md)), la categoría de sistema perdió su única razón de existir y se degradó a categoría normal; el registro ya no la crea. La pregunta solo aplicaría si `Transaction` volviera a exponerse.

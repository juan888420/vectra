...
interface ScenarioCategoryGalleryProps {
  ...
  /** Whether "creating a category" is the active step — swaps the hint copy
   * and highlights the tile like any other selected one, so the gallery
   * reads as "this is what's active" rather than losing its selection. */
 ...
 * than as a list (RFC-0025). The trailing "+" tile creates one inline instead
 * of sending the user away to the Categories screen. */
export function ScenarioCategoryGallery({
  ...
  const selected = creatingCategory
    ? null
    ...
    <div className="flex flex-col items-center gap-3">
      <div className="flex flex-wrap justify-center gap-2">
        ...
        <button
          ...
          className={cn(
            "relative flex size-14 items-center justify-center rounded-xl border border-dashed transition-colors",
            creatingCategory
              ? "border-primary bg-primary text-primary-foreground"
